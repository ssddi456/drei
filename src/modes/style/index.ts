...
export function getCSSMode(documentRegions: LanguageModelCache<SanDocumentRegions>): LanguageMode {
    const languageService = getCSSLanguageService();
    return getStyleMode('css', languageService, documentRegions);
}

export function getPostCSSMode(documentRegions: LanguageModelCache<SanDocumentRegions>): LanguageMode {
    const languageService = getCSSLanguageService();
    return getStyleMode('postcss', languageService, documentRegions);
}

export function getSCSSMode(documentRegions: LanguageModelCache<SanDocumentRegions>): LanguageMode {
    const languageService = getSCSSLanguageService();
    return getStyleMode('scss', languageService, documentRegions);
}
export function getLESSMode(documentRegions: LanguageModelCache<SanDocumentRegions>): LanguageMode {
...
function getStyleMode(
    ...
    return {
        ...
        doComplete(document, position) {
            ...
            const emmetCompletions: CompletionList = emmet.doComplete(document, position, emmetSyntax, config.emmet);
        ...
        format(document, currRange, formattingOptions) {
            if (config.drei.format.defaultFormatter[languageId] === 'none') {
                return [];
            }

            const { value, range } = getValueAndRange(document, currRange);
            const needIndent = config.drei.format.styleInitialIndent;
