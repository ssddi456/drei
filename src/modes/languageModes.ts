...
export function getLanguageModes(workspacePath: string | null | undefined): LanguageModes {
  const documentRegions = getLanguageModelCache<SanDocumentRegions>(10, 60, document => getDocumentRegions(document));
  ...
  let modes: { [k: string]: LanguageMode } = {
    san: getSanMode(),
    'san-html': getSanHTMLMode(documentRegions, workspacePath, jsMode),
